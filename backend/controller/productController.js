...
exports.createProduct = asyncWrapper(async (req, res) => {
  ...
  const { sizes, customMeasurements, ...rest } = req.body;

  // Add user ID to the product data
  rest.user = req.user.id;

  const data = await ProductModel.create({...rest, sizes, customMeasurements});
...
exports.getAllProducts = asyncWrapper(async (req, res) => {
  const resultPerPage = 12; // Number of products visible per page
...
exports.updateProduct = asyncWrapper(async (req, res, next) => {
  ...
  if (images !== undefined) {
    ...
    // Extract sizes from request body
    const { sizes, ...rest } = req.body;

  product = await ProductModel.findByIdAndUpdate(req.params.id, {...rest, sizes}, {
    new: true,
    runValidators: true,
